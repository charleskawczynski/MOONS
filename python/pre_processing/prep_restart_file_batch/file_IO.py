...
def line_N(filename,N):
	fh = open(filename, 'rb')
	fh.seek(-1024, 2)
	last = fh.readlines()[N].decode()
	return last
	# return [float(i) for i in last.split()]
...
def line_starts_with(f,s_match):
	for k in range(0,len(f)):
		if f[k].startswith(s_match):
			return k+1

def modify_param(root,file_set,val_set,param):
	print '\nMODIFYING PARAMETER '+param
	for f,v in zip(file_set,val_set):
		print '\n'+f.replace(root,'')
		fc = get_file_contents(f)
		l_match = line_starts_with(fc,' '+param)
		print param + ' = ' + str(v)
		fc[l_match] = ' ' + str(v) + '\n'
		set_file_contents(f,fc)
...
def set_file_contents(f,data):
	with open(f,'w') as file:
		file.writelines(data)
	return data

def get_data(file):
	arr = np.loadtxt(file,skiprows=3) # Get data
	return arr

def get_data_new(file):
	arr = pd.loadtxt(file,skiprows=3) # Get data
	return arr
