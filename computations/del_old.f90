      ...
      subroutine diff(dfdh,f,g,dir,n,diffType,s,genType)
        ...
        case (1); call collocatedAssign(dfdh,f,g%c(dir)%dhc,g%c(dir)%dhn,n,s,1)    ! Collocated CellCenter derivative
        case (2); call collocatedAssign(dfdh,f,g%c(dir)%dhn,g%c(dir)%dhc,n,s,0)    ! Collocated Node derivative
        case (3); call staggeredAssign(dfdh,f,g%c(dir)%dhc,s,1)           ! Staggered derivative (CC->N)
        case (4); call staggeredAssign(dfdh,f,g%c(dir)%dhn,s,0)           ! Staggered derivative (N->CC)
        end select
        case (2) ! add
        select case (diffType)
        case (1); call collocatedAdd(dfdh,f,g%c(dir)%dhc,g%c(dir)%dhn,n,s,1)       ! Collocated CellCenter derivative
        case (2); call collocatedAdd(dfdh,f,g%c(dir)%dhn,g%c(dir)%dhc,n,s,0)       ! Collocated Node derivative
        case (3); call staggeredAdd(dfdh,f,g%c(dir)%dhc,s,1)              ! Staggered derivative (CC->N)
        case (4); call staggeredAdd(dfdh,f,g%c(dir)%dhn,s,0)              ! Staggered derivative (N->CC)
        end select
        case (3) ! subtract
        select case (diffType)
        case (1); call collocatedSubtract(dfdh,f,g%c(dir)%dhc,g%c(dir)%dhn,n,s,1)  ! Collocated CellCenter derivative
        case (2); call collocatedSubtract(dfdh,f,g%c(dir)%dhn,g%c(dir)%dhc,n,s,0)  ! Collocated Node derivative
        case (3); call staggeredSubtract(dfdh,f,g%c(dir)%dhc,s,1)         ! Staggered derivative (CC->N)
        case (4); call staggeredSubtract(dfdh,f,g%c(dir)%dhn,s,0)         ! Staggered derivative (N->CC)
        end select
        case (4) ! multiply
        select case (diffType)
        case (1); call collocatedMultiply(dfdh,f,g%c(dir)%dhc,g%c(dir)%dhn,n,s,1)  ! Collocated CellCenter derivative
        case (2); call collocatedMultiply(dfdh,f,g%c(dir)%dhn,g%c(dir)%dhc,n,s,0)  ! Collocated Node derivative
        case (3); call staggeredMultiply(dfdh,f,g%c(dir)%dhc,s,1)         ! Staggered derivative (CC->N)
        case (4); call staggeredMultiply(dfdh,f,g%c(dir)%dhn,s,0)         ! Staggered derivative (N->CC)
        end select
        case (5) ! divide
        select case (diffType)
        case (1); call collocatedDivide(dfdh,f,g%c(dir)%dhc,g%c(dir)%dhn,n,s,1)    ! Collocated CellCenter derivative
        case (2); call collocatedDivide(dfdh,f,g%c(dir)%dhn,g%c(dir)%dhc,n,s,0)    ! Collocated Node derivative
        case (3); call staggeredDivide(dfdh,f,g%c(dir)%dhc,s,1)           ! Staggered derivative (CC->N)
        case (4); call staggeredDivide(dfdh,f,g%c(dir)%dhn,s,0)           ! Staggered derivative (N->CC)
        end select
        case default
