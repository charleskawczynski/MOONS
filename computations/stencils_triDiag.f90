      ...
      interface staggered;     module procedure staggeredO2_new;                     end interface
      ...
      function staggeredO2_new(f,D,U,s,gt) result(dfdh)
        ...
        dfdh(1+gt:s-1+gt) = (/(f(i+1)*U(i)-f(i)*D(i),i=1,s-1)/)
      end function

      function collocatedDfDh(f,L,D,U,s,gt) result(dfdh)
        implicit none
        real(cp),dimension(s) :: dfdh
        real(cp),intent(in),dimension(:) :: f
        real(cp),intent(in),dimension(:) :: L,D,U
        integer,intent(in) :: gt,s
        integer :: i,j,k
        ! Interior
        do i=2,s-1
          dfdh(i) = f(i-1)*L(i-1) + f(i)*D(i) + f(i+1)*U(i)
        enddo
