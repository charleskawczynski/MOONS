       ...
       subroutine interpO2(f,q,grid,dir,gridType) ! Seems to be working.
         ...
         ! 
         implicit none
         real(dpn),dimension(:,:,:),intent(in) :: q
         ...
         integer,dimension(3) :: sq,sf
         sq = shape(q); sf = shape(f)

         if (gridType.eq.2) then
           allocate(hn(sq(dir)-1)); hn = grid%c(dir)%hn
           allocate(hc(sq(dir)));   hc = grid%c(dir)%hc
         endif
         ...
         case (1) ! f(cc gridType), g(node/face gridType)
         !            f  g  f  g  f  g  f
         !            o--|--o--|--o--|--o   --> dir
         !                  *     *
         ...
         do k=1,sq(3)-z
           do j=1,sq(2)-y
             do i=1,sq(1)-x
               f(i+x,j+y,k+z) = (q(i,j,k)+q(i+x,j+y,k+z))/real(2.0,cp)
             enddo
           enddo
         enddo
         !$OMP END PARALLEL DO
         case (2) ! f(node/face gridType), g(cc gridType)
         !            g  f  g  f  g  f  g
         !            o--|--o--|--o--|--o      --> dir
         !               *     *     *
         ...
         do k=1,sq(3)-z
           do j=1,sq(2)-y
             do i=1,sq(1)-x
               ...
               f(i,j,k) = q(i,j,k)*alpha +&
                             q(i+x,j+y,k+z)*(real(1.0,cp)-alpha)
             enddo
           enddo
         enddo
         !$OMP END DO
       ...
       subroutine extrapO2(f,q,grid,dir) ! Seems to be working.
         ! extrapO2 extrapolates q from the primary grid to the
