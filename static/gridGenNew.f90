       ...
       type gridGen
         private
         type(grid) :: g
         integer,dimension(3) :: nseg                  ! Number of segments along x,y,z
         real(cp),dimension(:),allocatable :: hmin,hmax,N,alpha,beta
         logical,dimension(:),allocatable :: reverse   ! Reverse direction of stretching
       end type
      ...
      subroutine initGridGen(gg,nseg)
        implicit none
        type(gridGen),intent(inout) :: gg
        integer :: i
        call delete(gg)
        gg%nseg = nseg
      end subroutine
      ...
      subroutine deleteGridGen(gg)
        implicit none
        type(gridGen),intent(inout) :: gg
        if (allocate(gg%alpha)) deallocate(gg%alpha)
        if (allocate(gg%beta)) deallocate(gg%beta)
        if (allocate(gg%hmin)) deallocate(gg%hmin)
        if (allocate(gg%hmax)) deallocate(gg%hmax)
        if (allocate(gg%N)) deallocate(gg%N)
        if (allocate(gg%reverse)) deallocate(gg%reverse)
      end subroutine
      ...
      subroutine reverseIndex(h)
        ...
        integer :: i,s
        s = size(h); allocate(temp(s))
        do i=1,s; temp(s-i+1) = h(i); enddo
        h = temp; deallocate(temp)
      end subroutine

      function uniformGrid(hmin,hmax,N) result(hn)
        ! This routine returns a uniform grid from
        ! hmin to hmax using N segments (N+1 nodes)
        ! 
      ...
      function robertsGrid(hmin,hmax,N,alpha,beta,reverse) result(hn)
        ...
        hn = (/( ((b+two*a)*(g)**((hnbar(i)-a)/(real(1.0,cp)-a))-&
        b+two*a)/((two*a+real(1.0,cp))*(real(1.0,cp)+&
        g**((hnbar(i)-a)/(real(1.0,cp)-a)))),i=1,N+1)/)
        ! Return coordinates to original scale:
        hn = hmin + (hmax - hmin)*hn
        if (reverse) call reverseIndex(hn)
      end function

      function robertGrid(hmin,hmax,N,alpha,beta,reverse) result(hn)
       ! robertGrid returns the roberstGrid but removes the first index.
       ! This is useful when appending segments together.
       implicit none
       real(cp),intent(in) :: hmin,hmax
       integer,intent(in) :: N
       real(cp),intent(in) :: alpha,beta
       logical,intent(in) :: reverse
       real(cp),dimension(N+1) :: htemp
       real(cp),dimension(N) :: hn
       htemp = robertsGrid(hmin,hmax,N,alpha,beta,reverse)
       hn = htemp(2:N+1)
      end function

      subroutine setSegments(gg,hmin,hmax,N,alpha,beta,reverse)
       ! Segments must be set in increasing order (hn_1 < hn_2 < ... < hn_n)
       implicit none
       type(gridGen),intent(inout) :: gg
       real(cp),dimension(:),intent(in) :: hmin,hmax
       integer,dimension(:),intent(in) :: N
       real(cp),dimension(:),intent(in) :: alpha,beta
       logical,dimension(:),intent(in),optional :: reverse
       call delete(gg)
       allocate(gg%alpha(size(alpha)))
       allocate(gg%beta(size(beta)))
       allocate(gg%hmin(size(hmin)))
       allocate(gg%hmax(size(hmax)))
       allocate(gg%N(size(N)))
       gg%hmin = hmin; gg%hmax = hmax
       gg%alpha;  gg%beta = beta
       gg%N = N
       if (present(reverse)) then
         allocate(gg%reverse(size(reverse)))
         gg%reverse = reverse
       else
         allocate(gg%reverse(size(N)))
         gg%reverse = .false.
       endif
      end subroutine

      subroutine genGrid(gg,g,dir)
       implicit none
       type(gridGen),intent(inout) :: gg
       type(grid),intent(inout) :: g
       integer,intent(in) :: dir
       integer :: i
       call init(g%c(dir),(/gg%hmin(1),(robertGrid(gg%hmin(i),gg%hmax(i),gg%N(i),&
         gg%alpha(i),gg%beta(i),reverse(i)),i=1,gg%nseg(dir))/)
      end subroutine
       ...
       subroutine initGridGen(g_mom,g_ind,Re,Ha)
         ...
         call init(gg,(/1,1,1/))
         do d = 1,3
           call setSegments(gg,hmin(d),hmax(d),Ni(d),alphai(d),betai(d))
           call getGrid(gg,g_mom,d)
         enddo

         call init(gg,(/3,3,3/))
         do d = 1,3
           call setSegments(gg,(/hmin(d)-twbot(d),hmin(d),hmax(d)/),&
                               (/hmin(d),hmax(d),hmax(d)+twtop(d)/),&
                               (/Nwbot(d),Ni(d),Nwtop(d)/),&
                               (/alphaw(d),alphai(d),alphaw(d)/),&
                               (/betaw(d),betai(d),betaw(d)/),&
                               (/.false.,.false.,.true./))
           call getGrid(gg,g_ind,d)
         enddo
       ...
       subroutine robertsGridBL(beta,delta,hmin,hmax)
         ...
         integer :: i
         do i=1,3
           beta(i) = (real(1.0,cp) - delta/(hmax(i)-hmin(i)))**(-real(0.5,cp))
         enddo
       end subroutine
