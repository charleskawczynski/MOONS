      ...
      subroutine initSystem(ADI,s,g,gridType,dir)
        implicit none
        ...
        case (1) ! For cell-centered data
          select case (dir)
          case (1); allocate(dhn(s(1)-2)); dhn = g%c(1)%dhn
                    allocate(dhc(s(1)-1)); dhc = g%c(1)%dhc
          case (2); allocate(dhn(s(2)-2)); dhn = g%c(2)%dhn
                    allocate(dhc(s(2)-1)); dhc = g%c(2)%dhc
          case (3); allocate(dhn(s(3)-2)); dhn = g%c(3)%dhn
                    allocate(dhc(s(3)-1)); dhc = g%c(3)%dhc
          case default
            write(*,*) 'Error: dir must = 1,2,3 in initSystem.';stop
          end select
        case (2) ! For node-based data
          select case (dir)
          case (1); allocate(dhn(s(1)-1)); dhn = g%c(1)%dhn
                    allocate(dhc(s(1)));   dhc = g%c(1)%dhc
          case (2); allocate(dhn(s(2)-1)); dhn = g%c(2)%dhn
                    allocate(dhc(s(2)));   dhc = g%c(2)%dhc
          case (3); allocate(dhn(s(3)-1)); dhn = g%c(3)%dhn
                    allocate(dhc(s(3)));   dhc = g%c(3)%dhc
          case default
            write(*,*) 'Error: dir must = 1,2,3 in initSystem.';stop
          end select
        case default
        ...
        call setUpSystem(loDiag,diag,upDiag,real(0.5,cp)*ADI%dt*ADI%alpha,dhn,dhc,s(dir))
      ...
      subroutine initADI(ADI,s,g,gridType)
        ...
        h0 = minval((/g%c(1)%dhMin,g%c(2)%dhMin,g%c(3)%dhMin/))
        if (allocated(ADI%dtj)) deallocate(ADI%dtj)
        ADI%nTimeLevels = floor(log(real(smean+1))/log(real(2.0,cp)))
      ...
      subroutine relax3D(ADI,u,f,u_bcs,g)
        ...
        call interiorAdd(fstar,real(2.0,cp)*temp2,1)
        call interiorAdd(fstar,real(2.0,cp)*temp3,1)
      ...
      subroutine applyADI(ADI,u,f,u_bcs,g,ss,err,gridType,displayTF)
        ...
        call initSystem(ADI,s,g,gridType,1)
        call initSystem(ADI,s,g,gridType,2)
        call initSystem(ADI,s,g,gridType,3)
        ...
        if (displayTF) then
          ...
          case (1); call myCC2CCLap(ADI%lapu,u,g)
      ...
      subroutine solveADI(ADI,u,f,u_bcs,g,ss,err,gridType,displayTF)
        ...
        if (displayTF) then
          ...
          write(*,*) 'Number of ADI iterations = ',real(Nt,cp)*ADI%nTimeLevels
