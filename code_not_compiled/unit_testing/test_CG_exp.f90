       ...
       subroutine unit_test(dir)
         ...
         do i=1,f%RF(1)%s(1)
         ! f%RF(1)%f(i,:,:) = -(2.0_cp*PI)**2.0_cp*cos(2.0_cp*PI*m%g(1)%c(1)%hc(i))
         f%RF(1)%f(i,:,:) = -exp(-m%g(1)%c(1)%hc(i)**2.0_cp)
         enddo

         ! f%RF(1)%f(2,:,:) = 0.0_cp ! Dirichlet at x = 0
         ! f%RF(1)%f(f%RF(1)%s(1)-1,:,:) = 0.0_cp ! Dirichlet at x = 1
         ...
         call init(CG,Laplacian_uniform_props,m,MFP,u,temp_F,dir,'u',.false.,.true.)
         call assign(u,0.0_cp)
         ! do i=1,100
         call solve(CG,u,f,m,30,.true.)
         ! enddo
         call apply_BCs(u,m)
