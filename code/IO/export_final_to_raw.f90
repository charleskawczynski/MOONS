      ...
      subroutine export_3D_3C(m,U,dir,name,pad)
        ...
        do i=1,m%s
          ...
          call exp_3D_3C(U%x%RF(i)%s(1),U%x%RF(i)%s(2),U%x%RF(i)%s(3),&
                         U%x%RF(i)%f   ,U%y%RF(i)%f   ,U%z%RF(i)%f,&
                         get_h(U%x,m%g(i)%c(1),U%x%RF(i)%s(1)),&
                         get_h(U%x,m%g(i)%c(2),U%x%RF(i)%s(2)),&
                         get_h(U%x,m%g(i)%c(3),U%x%RF(i)%s(3)),&
                         fmt,pad,un)
        enddo
      ...
      subroutine export_2D_3C(m,U,dir,name,pad,direction)
        ...
        case (1)
          do i=1,m%s
            call exp_Zone_2I(un,U%x%RF(i)%s-2*pad,i)
            call exp_2D_3C(U%x%RF(i)%s(1),U%x%RF(i)%s(2),U%x%RF(i)%s(3),&
                           U%x%RF(i)%f   ,U%y%RF(i)%f   ,U%z%RF(i)%f,&
                           get_h(U%x,m%g(i)%c(2),U%x%RF(i)%s(2)),&
                           get_h(U%x,m%g(i)%c(3),U%x%RF(i)%s(3)),&
                           fmt,pad,un)
      ...
      subroutine export_1D_3C(m,U,dir,name,pad,axis)
        ...
        case (1)
          do i=1,m%s
            call exp_Zone_1I(un,U%x%RF(i)%s-2*pad,i)
            call exp_1D_3C(U%x%RF(i)%s(1),U%x%RF(i)%s(2),U%x%RF(i)%s(3),&
                           U%x%RF(i)%f   ,U%y%RF(i)%f   ,U%z%RF(i)%f,&
                           get_h(U%x,m%g(i)%c(1),U%x%RF(i)%s(1)),&
                           fmt,pad,un)
          enddo
