      ...
      use grid_mod
      use SF_mod
      use stencils_old_mod
      implicit none

      private
      public :: del_old,diff_old
      ...
      subroutine diff_old(dfdh,f,dhc,dhn,n,diffType,s,sdfdh,genType)
        implicit none
        real(cp),dimension(s),intent(in) :: f
        real(cp),dimension(:),intent(in) :: dhc,dhn
        integer,intent(in) :: n,diffType,s,sdfdh,genType
        ...
        case (3); call staggeredAssign(dfdh,f,dhc,s,sdfdh,1)     ! Staggered derivative (CC->N)
        case (4); call staggeredAssign(dfdh,f,dhn,s,sdfdh,0)     ! Staggered derivative (N->CC)
        end select
        case (2) ! add
        select case (diffType)
        ...
        case (3); call staggeredAdd(dfdh,f,dhc,s,sdfdh,1)        ! Staggered derivative (CC->N)
        case (4); call staggeredAdd(dfdh,f,dhn,s,sdfdh,0)        ! Staggered derivative (N->CC)
        end select
        case (3) ! subtract
        select case (diffType)
        ...
        case (3); call staggeredSubtract(dfdh,f,dhc,s,sdfdh,1)   ! Staggered derivative (CC->N)
        case (4); call staggeredSubtract(dfdh,f,dhn,s,sdfdh,0)   ! Staggered derivative (N->CC)
        end select
        case (4) ! multiply
        select case (diffType)
        ...
        case (3); call staggeredMultiply(dfdh,f,dhc,s,sdfdh,1)   ! Staggered derivative (CC->N)
        case (4); call staggeredMultiply(dfdh,f,dhn,s,sdfdh,0)   ! Staggered derivative (N->CC)
        end select
        case (5) ! divide
        select case (diffType)
        ...
        case (3); call staggeredDivide(dfdh,f,dhc,s,sdfdh,1)     ! Staggered derivative (CC->N)
        case (4); call staggeredDivide(dfdh,f,dhn,s,sdfdh,0)     ! Staggered derivative (N->CC)
        end select
        case default
      ...
      subroutine staggeredAssign(dfdh,f,dh,s,sdfdh,gt); implicit none
        real(cp),dimension(s),intent(in) :: f
        real(cp),dimension(:),intent(in) :: dh
        real(cp),dimension(sdfdh),intent(inout) :: dfdh
        integer,intent(in) :: s,sdfdh,gt
        dfdh = staggered_old(f,dh,s,sdfdh,gt)
      end subroutine

      subroutine staggeredAdd(dfdh,f,dh,s,sdfdh,gt); implicit none
        real(cp),dimension(s),intent(in) :: f
        real(cp),dimension(:),intent(in) :: dh
        real(cp),dimension(sdfdh),intent(inout) :: dfdh
        integer,intent(in) :: s,sdfdh,gt
        dfdh = dfdh + staggered_old(f,dh,s,sdfdh,gt)
      end subroutine

      subroutine staggeredSubtract(dfdh,f,dh,s,sdfdh,gt); implicit none
        real(cp),dimension(s),intent(in) :: f
        real(cp),dimension(:),intent(in) :: dh
        real(cp),dimension(sdfdh),intent(inout) :: dfdh
        integer,intent(in) :: s,sdfdh,gt
      ...
      subroutine staggeredMultiply(dfdh,f,dh,s,sdfdh,gt); implicit none
        real(cp),dimension(s),intent(in) :: f
        real(cp),dimension(:),intent(in) :: dh
        real(cp),dimension(sdfdh),intent(inout) :: dfdh
        integer,intent(in) :: s,sdfdh,gt
      ...
      subroutine staggeredDivide(dfdh,f,dh,s,sdfdh,gt); implicit none
        real(cp),dimension(s),intent(in) :: f
        real(cp),dimension(:),intent(in) :: dh
        real(cp),dimension(sdfdh),intent(inout) :: dfdh
        integer,intent(in) :: s,sdfdh,gt
      ...
      subroutine collocatedAssign(dfdh,f,dhp,dhd,n,s,gt); implicit none
        ...
        select case (n)
        case (1); dfdh = collocated_old(f,dhp,s,gt)
        case (2); dfdh = collocated_old(f,dhp,dhd,s,gt)
        end select
      end subroutine
      ...
      subroutine collocatedAdd(dfdh,f,dhp,dhd,n,s,gt); implicit none
        ...
        integer,intent(in) :: n,s,gt
        select case (n)
        case (1); dfdh = dfdh + collocated_old(f,dhp,s,gt)
        case (2); dfdh = dfdh + collocated_old(f,dhp,dhd,s,gt)
        end select
      end subroutine
      ...
      subroutine delGen_RF(dfdh,f,g,n,dir,pad,genType,s,sdfdh)
        ...
        case (1)
          ...
          do k=1+pad,s(3)-pad; do j=1+pad,s(2)-pad
              call diff_old(dfdh(:,j,k),f(:,j,k),g%c(dir)%dhc,g%c(dir)%dhn,n,diffType,s(1),sdfdh(1),genType)
          enddo; enddo
          !$OMP END PARALLEL DO
        case (2)
          !$OMP PARALLEL DO
          do k=1+pad,s(3)-pad; do i=1+pad,s(1)-pad
              call diff_old(dfdh(i,:,k),f(i,:,k),g%c(dir)%dhc,g%c(dir)%dhn,n,diffType,s(2),sdfdh(2),genType)
          enddo; enddo
          !$OMP END PARALLEL DO
        case (3)
          !$OMP PARALLEL DO
          do j=1+pad,s(2)-pad; do i=1+pad,s(1)-pad
              call diff_old(dfdh(i,j,:),f(i,j,:),g%c(dir)%dhc,g%c(dir)%dhn,n,diffType,s(3),sdfdh(3),genType)
          enddo; enddo
          !$OMP END PARALLEL DO
        case default
