       ...
       ! 
       ! Pre-processor directives: (_PARALLELIZE_TRADE_PHYSICAL_,_DEBUG_TRADE_PHYSICAL_)
       ...
       interface EE;             module procedure trade_physical_RF;   end interface
       ...
       subroutine trade_physical_RF_raw(A,B,A1,A2,B1,B2)
...
#ifdef _PARALLELIZE_trade_physical_
         integer :: i,j,k
       ...
       subroutine trade_physical_RF(A,B,AB,iA,iB)
         ! This is the embed/extract (EE) routine.
         implicit none
         type(realField),intent(inout) :: A
         type(realField),intent(in) :: B
         ...
         call trade_physical_RF_raw(A,B,(/AB(1)%R1(iA),AB(2)%R1(iA),AB(3)%R1(iA)/),&
       ...
       subroutine trade_physical_VF(U,D)
         implicit none
         type(VF),intent(inout) :: U
         type(domain),intent(in) :: D
         call trade_physical(U%x,D)
         call trade_physical(U%y,D)
         call trade_physical(U%z,D)
       end subroutine
       subroutine trade_physical_SF(U,D)
         implicit none
         type(SF),intent(inout) :: U
         type(domain),intent(in) :: D
         if (total%is_CC) then;       call extractCC(interior,total,D)
         elseif (total%is_Node) then; ! call extractNode(interior,total,D)
         stop 'Error: N not supported in extract_SF in ops_trade_physical.f90'
         elseif (total%is_Face) then; call extractFace(interior,total,D)
         elseif (total%is_Edge) then; call extractEdge(interior,total,D)
         else; stop 'Error: bad data input to extract_F in trade_physical.f90'
         endif
       end subroutine
       ...
       subroutine extractCC_SF(CC_i,CC_t,D)
         ...
         type(domain),intent(in) :: D
         integer :: i
#ifdef _DEBUG_trade_physical_
         if (.not.CC_i%is_CC) stop 'Error: CC data not found (1) in extractCC_SF in ops_trade_physical.f90'
         if (.not.CC_t%is_CC) stop 'Error: CC data not found (2) in extractCC_SF in ops_trade_physical.f90'
#endif
         do i=1,D%s
       ...
       subroutine extractCC_VF(CC_i,CC_t,D)
         ...
         integer :: i
#ifdef _DEBUG_trade_physical_
         if (.not.CC_i%is_CC) stop 'Error: CC data not found (1) in extractCC_VF in ops_trade_physical.f90'
         if (.not.CC_t%is_CC) stop 'Error: CC data not found (2) in extractCC_VF in ops_trade_physical.f90'
#endif
         do i=1,D%s
       ...
       subroutine extractFace_SF(face_i,face_t,D) ! Extracts Lorentz force from induction to momentum
         ...
         integer :: i
#ifdef _DEBUG_trade_physical_
         if (.not.face_t%is_Face) stop 'Error: face data not found (1) in extractFace_SF in ops_trade_physical.f90'
         if (.not.face_i%is_Face) stop 'Error: face data not found (2) in extractFace_SF in ops_trade_physical.f90'
#endif
         do i=1,D%s
       ...
       subroutine extractFace_VF(face_i,face_t,D) ! Extracts Lorentz force from induction to momentum
         ...
         integer :: i
#ifdef _DEBUG_trade_physical_
         if (.not.face_t%is_Face) stop 'Error: face data not found (1) in extractFace_VF in ops_trade_physical.f90'
         if (.not.face_i%is_Face) stop 'Error: face data not found (2) in extractFace_VF in ops_trade_physical.f90'
#endif
         do i=1,D%s
       ...
       subroutine extractEdge_SF(edge_i,edge_t,D) ! Auxiliary (energy budget)
         ...
         integer :: i
#ifdef _DEBUG_trade_physical_
         if (.not.edge_i%is_Edge) stop 'Error: edge data not found (1) in extractEdge_SF in ops_trade_physical.f90'
         if (.not.edge_t%is_Edge) stop 'Error: edge data not found (2) in extractEdge_SF in ops_trade_physical.f90'
#endif
         do i=1,D%s
       ...
       subroutine extractEdge_VF(edge_i,edge_t,D) ! Auxiliary (energy budget)
         ...
         integer :: i
#ifdef _DEBUG_trade_physical_
         if (.not.edge_i%is_Edge) stop 'Error: edge data not found (1) in extractEdge_VF in ops_trade_physical.f90'
         if (.not.edge_t%is_Edge) stop 'Error: edge data not found (2) in extractEdge_VF in ops_trade_physical.f90'
#endif
         do i=1,D%s
       ...
       subroutine embed_SF(total,interior,D)
         ...
         stop 'Error: N not supported in embed_SF in ops_trade_physical.f90'
       ...
       subroutine embedCC_SF(CC_t,CC_i,D) ! For material properties
         ...
         integer :: i
#ifdef _DEBUG_trade_physical_
         if (.not.CC_i%is_CC) stop 'Error: CC data not found (1) in embedCC_SF in ops_trade_physical.f90'
         if (.not.CC_t%is_CC) stop 'Error: CC data not found (2) in embedCC_SF in ops_trade_physical.f90'
#endif
         do i=1,D%s
       ...
       subroutine embedCC_VF(CC_t,CC_i,D)
         ...
         integer :: i
#ifdef _DEBUG_trade_physical_
         if (.not.CC_i%is_CC) stop 'Error: CC data not found (1) in embedCC_VF in ops_trade_physical.f90'
         if (.not.CC_t%is_CC) stop 'Error: CC data not found (2) in embedCC_VF in ops_trade_physical.f90'
#endif
         do i=1,D%s
       ...
       subroutine embedFace_SF(Face_t,Face_i,D)
         ...
         integer :: i
#ifdef _DEBUG_trade_physical_
         if (.not.Face_i%is_Face) stop 'Error: Face data not found (1) in embedFace_SF in ops_trade_physical.f90'
         if (.not.Face_t%is_Face) stop 'Error: Face data not found (2) in embedFace_SF in ops_trade_physical.f90'
#endif
         do i=1,D%s
       ...
       subroutine embedFace_VF(Face_t,Face_i,D)
         ...
         integer :: i
#ifdef _DEBUG_trade_physical_
         if (.not.Face_i%is_Face) stop 'Error: Face data not found (1) in embedFace_VF in ops_trade_physical.f90'
         if (.not.Face_t%is_Face) stop 'Error: Face data not found (2) in embedFace_VF in ops_trade_physical.f90'
#endif
         do i=1,D%s
       ...
       subroutine embedEdge_SF(Edge_t,Edge_i,D) ! Embeds velocity from momentum into induction
         ...
         integer :: i
#ifdef _DEBUG_trade_physical_
         if (.not.edge_t%is_Edge) stop 'Error: edge data not found (1) in embedEdge_SF in ops_trade_physical.f90'
         if (.not.edge_i%is_Edge) stop 'Error: edge data not found (2) in embedEdge_SF in ops_trade_physical.f90'
#endif
         do i=1,D%s
       ...
       subroutine embedEdge_VF(Edge_t,Edge_i,D) ! Embeds velocity from momentum into induction
         ...
         integer :: i
#ifdef _DEBUG_trade_physical_
         if (.not.edge_t%is_Edge) stop 'Error: edge data not found (1) in embedEdge_VF in ops_trade_physical.f90'
         if (.not.edge_i%is_Edge) stop 'Error: edge data not found (2) in embedEdge_VF in ops_trade_physical.f90'
#endif
         do i=1,D%s
       ...
       !   type(overlap),dimension(3) :: GC
       !   type(overlap),dimension(3) :: GN
       ...
       function EE_shape(f,D,i) result(s)
         ...
         if (f%is_Face) then
           select case (f%face)
           case (1); s = (/D%sd(i)%GN(1),D%sd(i)%GC(2),D%sd(i)%GC(3)/)
           case (2); s = (/D%sd(i)%GC(1),D%sd(i)%GN(2),D%sd(i)%GC(3)/)
           case (3); s = (/D%sd(i)%GC(1),D%sd(i)%GC(2),D%sd(i)%GN(3)/)
           case default; stop 'Error: f%face must = 1,2,3 in ops_trade_physical.f90'
           end select
         elseif (f%is_Edge) then
           select case (f%edge)
           case (1); s = (/D%sd(i)%GC(1),D%sd(i)%GN(2),D%sd(i)%GN(3)/)
           case (2); s = (/D%sd(i)%GN(1),D%sd(i)%GC(2),D%sd(i)%GN(3)/)
           case (3); s = (/D%sd(i)%GN(1),D%sd(i)%GN(2),D%sd(i)%GC(3)/)
           case default; stop 'Error: f%edge must = 1,2,3 in ops_trade_physical.f90'
           end select
         elseif (f%is_CC) then
           s = (/D%sd(i)%GC(1),D%sd(i)%GC(2),D%sd(i)%GC(3)/)
         elseif (f%is_Node) then
           s = (/D%sd(i)%GN(1),D%sd(i)%GN(2),D%sd(i)%GN(3)/)
         else; stop 'Error: no type found in ops_trade_physical.f90'
         endif
       end function
