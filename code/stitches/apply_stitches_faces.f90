       ...
       subroutine adjust_bounds(U,m,i,j,k,a,b,x,y,z)
         ...
         if (U%BF(i)%GF%b%defined) then ! Satisfies BCs for 2D flow over a square but fails for 9 domains
         a = (/2+x,2+y,2+z/); b = (/U%BF(i)%GF%s(1)-1-x,U%BF(i)%GF%s(2)-1-y,U%BF(i)%GF%s(3)-1-z/)
         fa = adj_faces_given_dir(k)

         d = dir_given_face(fa(1))
         L(1) = m%B(i)%g%st_faces(fa(1))%TF.and.m%B(j)%g%st_faces(fa(1))%TF
         L(2) = .not.m%B(i)%g%st_faces(fa(1))%TF.and.is_Neumann(U%BF(i)%GF%b%f(fa(1))%b)
         L(3) = .not.m%B(j)%g%st_faces(fa(1))%TF.and.is_Neumann(U%GF(j)%b%f(fa(1))%b)
         if ((k.ne.d).and.U%N_along(d).and.(L(1).or.(L(2).and.L(3)))) a(d) = a(d)-1
         ! if (L(2).and.L(3)) a(d) = 1

         d = dir_given_face(fa(2))
         L(1) = m%B(i)%g%st_faces(fa(2))%TF.and.m%B(j)%g%st_faces(fa(2))%TF
         L(2) = .not.m%B(i)%g%st_faces(fa(2))%TF.and.is_Neumann(U%BF(i)%GF%b%f(fa(2))%b)
         L(3) = .not.m%B(j)%g%st_faces(fa(2))%TF.and.is_Neumann(U%GF(j)%b%f(fa(2))%b)
         if ((k.ne.d).and.U%N_along(d).and.(L(1).or.(L(2).and.L(3)))) b(d) = b(d)+1
         ! if (L(2).and.L(3)) b(d) = U%BF(i)%GF%s(d)

         d = dir_given_face(fa(3))
         L(1) = m%B(i)%g%st_faces(fa(3))%TF.and.m%B(j)%g%st_faces(fa(3))%TF
         L(2) = .not.m%B(i)%g%st_faces(fa(3))%TF.and.is_Neumann(U%BF(i)%GF%b%f(fa(3))%b)
         L(3) = .not.m%B(j)%g%st_faces(fa(3))%TF.and.is_Neumann(U%GF(j)%b%f(fa(3))%b)
         if ((k.ne.d).and.U%N_along(d).and.(L(1).or.(L(2).and.L(3)))) a(d) = a(d)-1
         ! if (L(2).and.L(3)) a(d) = 1

         d = dir_given_face(fa(4))
         L(1) = m%B(i)%g%st_faces(fa(4))%TF.and.m%B(j)%g%st_faces(fa(4))%TF
         L(2) = .not.m%B(i)%g%st_faces(fa(4))%TF.and.is_Neumann(U%BF(i)%GF%b%f(fa(4))%b)
         L(3) = .not.m%B(j)%g%st_faces(fa(4))%TF.and.is_Neumann(U%GF(j)%b%f(fa(4))%b)
         if ((k.ne.d).and.U%N_along(d).and.(L(1).or.(L(2).and.L(3)))) b(d) = b(d)+1
         ! if (L(2).and.L(3)) b(d) = U%BF(i)%GF%s(d)
         ...
         fa = adj_faces_given_dir(k)
         d = dir_given_face(fa(1))
         if (U%N_along(d).and.(m%B(i)%g%st_faces(fa(1))%TF.and.m%B(j)%g%st_faces(fa(1))%TF)) a(d) = a(d)-1
         d = dir_given_face(fa(2))
         if (U%N_along(d).and.(m%B(i)%g%st_faces(fa(2))%TF.and.m%B(j)%g%st_faces(fa(2))%TF)) b(d) = b(d)+1
         d = dir_given_face(fa(3))
         if (U%N_along(d).and.(m%B(i)%g%st_faces(fa(3))%TF.and.m%B(j)%g%st_faces(fa(3))%TF)) a(d) = a(d)-1
         d = dir_given_face(fa(4))
         if (U%N_along(d).and.(m%B(i)%g%st_faces(fa(4))%TF.and.m%B(j)%g%st_faces(fa(4))%TF)) b(d) = b(d)+1
         endif
         if (m%B(i)%g%c(1)%N.eq.1) then; a(1) = 1; b(1) = U%BF(i)%GF%s(1); endif
         if (m%B(i)%g%c(2)%N.eq.1) then; a(2) = 1; b(2) = U%BF(i)%GF%s(2); endif
         if (m%B(i)%g%c(3)%N.eq.1) then; a(3) = 1; b(3) = U%BF(i)%GF%s(3); endif
       end subroutine
       ...
       subroutine apply_stitches_faces_SF(U,m)
         ...
         if (m%s.gt.1) then
           ...
           do i=1,m%s; do k=1,3
             ...
             if (m%B(i)%g%st_faces(f(1))%TF) then
               call adjust_bounds(U,m,i,m%B(i)%g%st_faces(f(1))%ID,k,a,b,x,y,z)
               call app_F(U%BF(i)%GF,U%GF(m%B(i)%g%st_faces(f(1))%ID),k,x,y,z,a,b)
             endif
           enddo; enddo
         endif
       end subroutine
       ...
       subroutine app_F(Umin,Umax,dir,x,y,z,a,b)
         ...
         case default
         stop 'Erorr: dir must = 1,2,3 in applyAllStitches_GF in apply_stitches_faces.f90'
         end select
       end subroutine
