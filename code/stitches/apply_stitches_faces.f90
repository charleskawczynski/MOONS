       ...
       subroutine adjust_bounds(U,m,i,j,k,a,b,x,y,z)
         ...
         !              - U%GF(i)%st_faces(f(p))%TF and U%GF(j)%st_faces(f(p))%TF are true
         !           or - U%GF(i)%b%f(p)%b%Neumann and U%GF(j)%b%f(p)%b%Neumann
         ! 
         ! Alternatively, stitching should NOT occur if
         !              - .not.U%GF(i)%st_faces(f(p))%TF and U%GF(i)%b%f(p)%b%Neumann
         !           or - .not.U%GF(j)%st_faces(f(p))%TF and U%GF(j)%b%f(p)%b%Neumann
         ! 
         ! a = 2; b = U%GF(i)%s-1 ! Works for 9 domains but fails to satisfy BCs for 2D flow over square
         ! a = 2; b = U%GF(i)%s-1 Need to get back here IF BCs of i and j are Neumann...

         if (U%GF(i)%b%defined) then ! Satisfies BCs for 2D flow over a square but fails for 9 domains
         a = (/2+x,2+y,2+z/); b = (/U%GF(i)%s(1)-1-x,U%GF(i)%s(2)-1-y,U%GF(i)%s(3)-1-z/)
         ...
         if (m%g(i)%c(1)%N.eq.1) then; a(1) = 1; b(1) = U%GF(i)%s(1); endif
         if (m%g(i)%c(2)%N.eq.1) then; a(2) = 1; b(2) = U%GF(i)%s(2); endif
         if (m%g(i)%c(3)%N.eq.1) then; a(3) = 1; b(3) = U%GF(i)%s(3); endif
       end subroutine
         ...
         if (m%s.gt.1) then
           ...
           do i=1,m%s; do k=1,3
             ...
             if (m%g(i)%st_faces(f(1))%TF) then
               ...
               call app_F(U%GF(i),U%GF(m%g(i)%st_faces(f(1))%ID),k,x,y,z,a,b)
             endif
           enddo; enddo
         endif
       end subroutine
