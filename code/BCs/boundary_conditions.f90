       ...
       type boundary_conditions
         ! BC values:
         type(block_domain) :: Dirichlet,Neumann,Periodic,Robin
       ...
       interface init;                module procedure init_GFs_BCs;            end interface
       ...
       subroutine init_GFs_BCs(BC,B)
         implicit none
         type(BCs),intent(inout) :: BC
         type(block),intent(in) :: B
         integer :: i
         do i=1,6;  call init(BC%f(i),B%f(i)); call assign(BC%f(i),0.0_cp); enddo
         do i=1,12; call init(BC%e(i),B%e(i)); call assign(BC%e(i),0.0_cp); enddo
         do i=1,8;  call init(BC%c(i),B%c(i)); call assign(BC%c(i),0.0_cp); enddo
         BC%GFs_defined = .true.
       ...
       subroutine init_BCs_copy(BC_out,BC_in)
         implicit none
         type(BCs),intent(inout) :: BC_out
         type(BCs),intent(in) :: BC_in
         integer :: i
