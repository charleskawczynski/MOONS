       ...
       function stitch_TF(RF,g,f) result(TF)
         ...
         logical :: TF
         TF_prep(1) = RF%b%f(f(1))%b%Dirichlet.and.(.not.g%st_faces(f(1)))
         TF_prep(2) = RF%b%f(f(2))%b%Dirichlet.and.(.not.g%st_faces(f(2)))
         TF = all(TF_prep)
       end function
       ...
       subroutine apply_edges_SF(U,m)
         ...
         if (U%is_CC) then
           ...
           f = adjacent_faces(e(4)); if (stitch_TF(U%RF(i),m%g(i),f)) call a_CC(U%RF(i),m%g(i),e(4),a(1),a(2),k,4)
           enddo; enddo
         elseif (U%is_Node) then
           ...
           f = adjacent_faces(e(4)); if (stitch_TF(U%RF(i),m%g(i),f)) call a_N(U%RF(i),m%g(i),e(4),a(1),a(2),k,4)
           enddo; enddo
         elseif (U%is_Face) then
           ...
           if (U%face.eq.k) then
             ...
             f = adjacent_faces(e(1)); if (stitch_TF(U%RF(i),m%g(i),f)) call a_F(U%RF(i),m%g(i),e(1),a(1),a(2),k,1)
             f = adjacent_faces(e(2)); if (stitch_TF(U%RF(i),m%g(i),f)) call a_F(U%RF(i),m%g(i),e(2),a(1),a(2),k,2)
             f = adjacent_faces(e(3)); if (stitch_TF(U%RF(i),m%g(i),f)) call a_F(U%RF(i),m%g(i),e(3),a(1),a(2),k,3)
             f = adjacent_faces(e(4)); if (stitch_TF(U%RF(i),m%g(i),f)) call a_F(U%RF(i),m%g(i),e(4),a(1),a(2),k,4)
           endif
           enddo; enddo
       ...
       subroutine a_CC(RF,g,e,d1,d2,dir,corner)
         ...
         call app_CC_RF(RF%f,RF%b%e(e)%v,RF%b%e(e)%b,RF%s(1),RF%s(2),RF%s(3),dir,g%c(d1),g%c(d2),corner)
       end subroutine
       ...
       subroutine a_N(RF,g,e,d1,d2,dir,corner)
         ...
         type(grid),intent(in) :: g
         integer,intent(in) :: dir,d1,d2,corner,e
         call app_N_RF(RF%f,RF%b%e(e)%v,RF%b%e(e)%b,RF%s(1),RF%s(2),RF%s(3),dir,g%c(d1),g%c(d2),corner)
       end subroutine

       subroutine a_F(RF,g,e,d1,d2,dir,corner)
         implicit none
         type(realField),intent(inout) :: RF
         type(grid),intent(in) :: g
         integer,intent(in) :: dir,d1,d2,corner,e
         call app_F_RF(RF%f,RF%b%e(e)%v,RF%b%e(e)%b,RF%s(1),RF%s(2),RF%s(3),dir,g%c(d1),g%c(d2),corner)
       end subroutine
       ...
       subroutine app_N_RF(f,v,bct,x,y,z,dir,c1,c2,corner)
         ...
         case (1); call a_N(bct,c1%dhn(1),c2%dhn(1),v,f(:,2,2),f(:,1,2),f(:,2,1),f(:,3,2),f(:,2,3))
         case (2); call a_N(bct,c1%dhn(1),c2%dhn(1),v,f(2,:,2),f(2,:,1),f(1,:,2),f(2,:,3),f(3,:,2))
         case (3); call a_N(bct,c1%dhn(1),c2%dhn(1),v,f(2,2,:),f(1,2,:),f(2,1,:),f(3,2,:),f(2,3,:))
         case default; stop 'Error: dir must = 1,2,3 in app_E_SF in apply_BCs_edges.f90'
         end select
         case (2)
         select case (dir) !                            ub        ug1       ug2         ui1         ui2
         case (1); call a_N(bct,c1%dhn(1),c2%dhn_e,v,f(:,2,z-1),f(:,1,z-1),f(:,2,z),f(:,3,z-1),f(:,2,z-2))
         case (2); call a_N(bct,c1%dhn(1),c2%dhn_e,v,f(x-1,:,2),f(x-1,:,1),f(x,:,2),f(x-1,:,3),f(x-2,:,2))
         case (3); call a_N(bct,c1%dhn(1),c2%dhn_e,v,f(2,y-1,:),f(1,y-1,:),f(2,y,:),f(3,y-1,:),f(2,y-2,:))
         case default; stop 'Error: dir must = 1,2,3 in app_E_SF in apply_BCs_edges.f90'
         end select
         case (3)
         select case (dir) !                           ub         ug1        ug2        ui1        ui2
         case (1); call a_N(bct,c1%dhn_e,c2%dhn(1),v,f(:,y-1,2),f(:,y,2),f(:,y-1,1),f(:,y-2,2),f(:,y-1,3))
         case (2); call a_N(bct,c1%dhn_e,c2%dhn(1),v,f(2,:,z-1),f(2,:,z),f(1,:,z-1),f(2,:,z-2),f(3,:,z-1))
         case (3); call a_N(bct,c1%dhn_e,c2%dhn(1),v,f(x-1,2,:),f(x,2,:),f(x-1,1,:),f(x-2,2,:),f(x-1,3,:))
         case default; stop 'Error: dir must = 1,2,3 in app_E_SF in apply_BCs_edges.f90'
         end select
         case (4)
         select case (dir) !                           ub       ug1        ug2         ui1         ui2
         case (1); call a_N(bct,c1%dhn_e,c2%dhn_e,v,f(:,y-1,z-1),f(:,y,z-1),f(:,y-1,z),f(:,y-2,z-1),f(:,y-1,z-2))
         case (2); call a_N(bct,c1%dhn_e,c2%dhn_e,v,f(x-1,:,z-1),f(x-1,:,z),f(x,:,z-1),f(x-1,:,z-2),f(x-2,:,z-1))
         case (3); call a_N(bct,c1%dhn_e,c2%dhn_e,v,f(x-1,y-1,:),f(x,y-1,:),f(x-1,y,:),f(x-2,y-1,:),f(x-1,y-2,:))
         case default; stop 'Error: dir must = 1,2,3 in app_E_SF in apply_BCs_edges.f90'
         end select
         case default; stop 'Error: corner must = 1,2,3,4 in app_E_SF in apply_BCs_edges.f90'
         end select
       ...
       subroutine app_F1_RF(f,v,bct,x,y,z,dir,c1,c2,corner)
         ! At this point, data should be sent in a convention so that the 
         ! next set of routines can handle applying BCs in a systematic way.
         ! Below are some notes and diagrams illustrating this process:
         ! 
         ...
         case (1); call a_F(bct,c1%dhc(1),c2%dhc(1),v,f(:,2,1),f(:,2,2))
         case (2); call a_F(bct,c1%dhc(1),c2%dhc(1),v,f(1,:,2),f(2,:,2))
         case (3); call a_F(bct,c1%dhc(1),c2%dhc(1),v,f(2,1,:),f(2,2,:))
         case default; stop 'Error: dir must = 1,2,3 in app_E_SF in apply_BCs_edges.f90'
         end select
         case (2)
         select case (dir) !                           ug        ui
         case (1); call a_CC(bct,c1%dhc(1),c2%dhc_e,v,f(:,2,z),f(:,2,z-1))
         case (2); call a_CC(bct,c1%dhc(1),c2%dhc_e,v,f(x,:,2),f(x-1,:,2))
         case (3); call a_CC(bct,c1%dhc(1),c2%dhc_e,v,f(2,y,:),f(2,y-1,:))
         case default; stop 'Error: dir must = 1,2,3 in app_E_SF in apply_BCs_edges.f90'
         end select
         case (3)
         select case (dir) !                           ug        ui
         case (1); call a_CC(bct,c1%dhc_e,c2%dhc(1),v,f(:,y-1,1),f(:,y-1,2))
         case (2); call a_CC(bct,c1%dhc_e,c2%dhc(1),v,f(1,:,z-1),f(2,:,z-1))
         case (3); call a_CC(bct,c1%dhc_e,c2%dhc(1),v,f(x-1,1,:),f(x-1,2,:))
         case default; stop 'Error: dir must = 1,2,3 in app_E_SF in apply_BCs_edges.f90'
         end select
         case (4)
         select case (dir) !                           ug        ui
         case (1); call a_CC(bct,c1%dhc_e,c2%dhc_e,v,f(:,y-1,z),f(:,y-1,z-1))
         case (2); call a_CC(bct,c1%dhc_e,c2%dhc_e,v,f(x,:,z-1),f(x-1,:,z-1))
         case (3); call a_CC(bct,c1%dhc_e,c2%dhc_e,v,f(x-1,y,:),f(x-1,y-1,:))
         case default; stop 'Error: dir must = 1,2,3 in app_E_SF in apply_BCs_edges.f90'
         end select
         case default; stop 'Error: corner must = 1,2,3,4 in app_E_SF in apply_BCs_edges.f90'
         end select
       ...
       subroutine app_F2_RF(f,v,bct,x,y,z,dir,c1,c2,corner)
         ! At this point, data should be sent in a convention so that the 
         ! next set of routines can handle applying BCs in a systematic way.
         ! Below are some notes and diagrams illustrating this process:
         ! 
         ...
         case (1)
         select case (dir) !                            ug        ui
         case (1); call a_F(bct,c1%dhc(1),c2%dhc(1),v,f(:,1,2),f(:,2,2))
         case (2); call a_F(bct,c1%dhc(1),c2%dhc(1),v,f(2,:,1),f(2,:,2))
         case (3); call a_F(bct,c1%dhc(1),c2%dhc(1),v,f(1,2,:),f(2,2,:))
         case default; stop 'Error: dir must = 1,2,3 in app_E_SF in apply_BCs_edges.f90'
         end select
         case (2)
         select case (dir) !                           ug        ui
         case (1); call a_CC(bct,c1%dhc(1),c2%dhc_e,v,f(:,1,z-1),f(:,2,z-1))
         case (2); call a_CC(bct,c1%dhc(1),c2%dhc_e,v,f(x-1,:,1),f(x-1,:,2))
         case (3); call a_CC(bct,c1%dhc(1),c2%dhc_e,v,f(1,y-1,:),f(2,y-1,:))
         case default; stop 'Error: dir must = 1,2,3 in app_E_SF in apply_BCs_edges.f90'
         end select
         case (3)
         select case (dir) !                           ug        ui
         case (1); call a_CC(bct,c1%dhc_e,c2%dhc(1),v,f(:,y,2),f(:,y-1,2))
         case (2); call a_CC(bct,c1%dhc_e,c2%dhc(1),v,f(2,:,z),f(2,:,z-1))
         case (3); call a_CC(bct,c1%dhc_e,c2%dhc(1),v,f(x,2,:),f(x-1,2,:))
         case default; stop 'Error: dir must = 1,2,3 in app_E_SF in apply_BCs_edges.f90'
         end select
         case (4)
         select case (dir) !                           ug        ui
         case (1); call a_CC(bct,c1%dhc_e,c2%dhc_e,v,f(:,y,z-1),f(:,y-1,z-1))
         case (2); call a_CC(bct,c1%dhc_e,c2%dhc_e,v,f(x-1,:,z),f(x-1,:,z-1))
         case (3); call a_CC(bct,c1%dhc_e,c2%dhc_e,v,f(x,y-1,:),f(x-1,y-1,:))
         case default; stop 'Error: dir must = 1,2,3 in app_E_SF in apply_BCs_edges.f90'
         end select
         case default; stop 'Error: corner must = 1,2,3,4 in app_E_SF in apply_BCs_edges.f90'
         end select
