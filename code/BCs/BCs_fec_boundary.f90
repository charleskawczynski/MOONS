       ...
       type BCs
         ...
         type(corner),dimension(8) :: c ! {2^3} = {ymin/zmin, xmin/zmin, xmin/ymin}
         type(grid) :: g
       ...
       subroutine init_BCType_face(b,face,bctype_n,bctype_cc)
         ...
         if (.not.b%gridDefined) stop 'Error: BC grid must be defined before type is defined'
         select case (face)
         case (1,2); if (b%s(1).eq.b%g%c(1)%sn) then; call init(b%f(face),bctype_n)
                 elseif (b%s(1).eq.b%g%c(1)%sc) then; call init(b%f(face),bctype_cc)
                 else; stop 'Error: size mismatch in init_BCType_face in BCs.f90'
                 endif
         case (3,4); if (b%s(2).eq.b%g%c(2)%sn) then; call init(b%f(face),bctype_n)
                 elseif (b%s(2).eq.b%g%c(2)%sc) then; call init(b%f(face),bctype_cc)
                 else; stop 'Error: size mismatch in init_BCType_face in BCs.f90'
                 endif
         case (5,6); if (b%s(3).eq.b%g%c(3)%sn) then; call init(b%f(face),bctype_n)
                 elseif (b%s(3).eq.b%g%c(3)%sc) then; call init(b%f(face),bctype_cc)
                 else; stop 'Error: size mismatch in init_BCType_face in BCs.f90'
                 endif
       ...
       subroutine init_BCType_edge(b,edge,bctype_n,bctype_cc)
         implicit none
         type(BCs),intent(inout) :: b
         integer,intent(in) :: edge,bctype_n,bctype_cc
         if (.not.b%gridDefined) stop 'Error: BC grid must be defined before type is defined'
         select case (edge)
         case (1,2); if (b%s(1).eq.b%g%c(1)%sn) then; call init(b%e(edge),bctype_n)
                 elseif (b%s(1).eq.b%g%c(1)%sc) then; call init(b%e(edge),bctype_cc)
                 else; stop 'Error: size mismatch in init_BCType_face in BCs.f90'
                 endif
         case (3,4); if (b%s(2).eq.b%g%c(2)%sn) then; call init(b%e(edge),bctype_n)
                 elseif (b%s(2).eq.b%g%c(2)%sc) then; call init(b%e(edge),bctype_cc)
                 else; stop 'Error: size mismatch in init_BCType_face in BCs.f90'
                 endif
         case (5,6); if (b%s(3).eq.b%g%c(3)%sn) then; call init(b%e(edge),bctype_n)
                 elseif (b%s(3).eq.b%g%c(3)%sc) then; call init(b%e(edge),bctype_cc)
                 else; stop 'Error: size mismatch in init_BCType_face in BCs.f90'
                 endif
         case default; stop 'Error: case not found in init_BCType_face in BCs.f90'
         end select
       ...
       subroutine init_vals_all_S(b,val)
         ...
         do i=1,12; call init(b%e(i),val); enddo
         do i=1,8;  call init(b%c(i),val); enddo
         call define_logicals(b)
       end subroutine
