       ...
       private
       public :: CT_method,init,delete,solve
       ...
       interface init;                 module procedure initCT_method;                 end interface
       interface delete;               module procedure deleteCT_method;               end interface
       interface solve;                module procedure solve_FiniteRem;               end interface
       ...
       subroutine initCT_method(CTM,g,sigma)
         implicit none
         type(CT_method),intent(inout) :: CTM
         type(grid),intent(in) :: g
         type(SF),intent(in) :: sigma
         type(SF) :: temp
         write(*,*) 'Initializing CT_method:'
       ...
       subroutine deleteCT_method(CTM)
         implicit none
         type(CT_method),intent(inout) :: CTM
         ...
         write(*,*) 'CT_method object deleted'
       end subroutine

       subroutine solve_FiniteRem(CTM,F_CC,U_E,g,Rem,dTime)
         ! Computes
         !    E = j/(Rem*sig) - uxB
         !    dBdt = -curl(E) + F
         !    B^n+1 = B^n + {-curl(E) + F}
       ...
       subroutine solve_LowRem(CTM,F_CC,U_E,g,Rem,dTime,NmaxB)
         ! Computes
         !    E = j/(Rem*sig) - uxB
         !    dBdt = -curl(E) + F
         !    B^n+1 = B^n + {-curl(E) + F}
