      ...
      subroutine Laplacian_uniform_props(Ax,x,vol,m,tempk,c,k)
        ! Computes:   A = ∇²
        implicit none
      ...
      subroutine B_diff_nat_stag_diag(Ax,x,vol,m,tempk,c,k,local_index)
        ...
        ! cell face and edge respectively. V is the cell volume.
        ! 
        ! Volume is not used, but suppressed warning would be expensive
        ! and format is very general to allow external procedure.
        implicit none
        type(VF),intent(inout) :: Ax
        type(VF),intent(in) :: x,vol
        type(VF),intent(in) :: k
