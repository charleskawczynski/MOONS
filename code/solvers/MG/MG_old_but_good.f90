      ...
      subroutine initMultiGrid(mg,s,u_bcs,g_base,ss,displayTF)
        ...
        do j = 2,mg(1)%nLevels
          ...
          if ((s(1).eq.g_base%c(1)%sn).and.(get_all_Neumann(mg(1)%u_bcs))) bctype = 4       ! Neumann wall coincident
          if ((s(1).eq.g_base%c(1)%sc).and.(get_all_Neumann(mg(1)%u_bcs))) bctype = 5       ! Neumann wall incoincident
          if ((s(1).eq.g_base%c(1)%sn).and.(.not.get_all_Neumann(mg(1)%u_bcs))) bctype = 1  ! Dirichlet wall coincident
          if ((s(1).eq.g_base%c(1)%sc).and.(.not.get_all_Neumann(mg(1)%u_bcs))) bctype = 2  ! Dirichlet wall incoincident
