      ...
      subroutine init_array_copy(A_out,A_in)
        implicit none
        type(arrayCP),intent(inout) :: A_out
        type(arrayCP),intent(in) :: A_in
        if (.not.allocated(A_in%f)) stop 'Error: input arrayCP is not allocated in arrayCP.f90'
        call delete(A_out)
        A_out%s = A_in%s
      ...
      subroutine print_array(A)
        ...
        if (allocated(A%f)) then
          ...
          stop 'Error: arrayCP is not allocated in print_array in arrayCP.f90'
        endif
      end subroutine
