      ...
      subroutine export_matrix_SF(D,m,dir,name)
        ...
        do i=1,un%numEl
          call unitVector_consecutive(un,i); call zeroGhostPoints(un)
      ...
      subroutine export_matrix_VF(D,m,dir,name)
        ...
        call assign(un,0.0_cp)
        do i=1,un%x%numEl
          call unitVector_consecutive(un%x,i); call zeroGhostPoints(un%x)
        ...
        call assign(un,0.0_cp)
        do i=1,un%y%numEl
          call unitVector_consecutive(un%y,i); call zeroGhostPoints(un%y)
        ...
        call assign(un,0.0_cp)
        do i=1,un%z%numEl
          call unitVector_consecutive(un%z,i); call zeroGhostPoints(un%z)
      ...
      subroutine export_operator_VF(operator,name,dir,x,vol,m,tempk,c,k)
        ...
        call assign(un,0.0_cp)
        do i=1,un%x%numEl
          call unitVector_consecutive(un%x,i); call zeroGhostPoints(un)
          if (sum(un%x).gt.0.5_cp) then
            call operator(Aun,un,vol,m,tempk,c,k)
        ...
        call assign(un,0.0_cp)
        do i=1,un%y%numEl
          call unitVector_consecutive(un%y,i); call zeroGhostPoints(un)
          if (sum(un%y).gt.0.5_cp) then
            call operator(Aun,un,vol,m,tempk,c,k)
            call export_transpose(Aun,newU) ! Export rows of A
          endif
        ...
        call assign(un,0.0_cp)
        do i=1,un%z%numEl
          call unitVector_consecutive(un%z,i); call zeroGhostPoints(un)
          if (sum(un%z).gt.0.5_cp) then
            call operator(Aun,un,vol,m,tempk,c,k)
            call export_transpose(Aun,newU) ! Export rows of A
          endif
      ...
      subroutine define_ith_diag_SF_old(D,Aun,col)
        implicit none
        type(SF),intent(inout) :: D
        ...
        if (row.eq.col) then; D%RF(t)%f(i,j,k) = Aun%RF(t)%f(i,j,k); exit; endif
        row = row + 1
        enddo; enddo; enddo; enddo
        !$OMP END PARALLEL DO
      end subroutine
      ...
      subroutine define_ith_diag_VF_old(D,Aun,col,component)
        implicit none
        type(VF),intent(inout) :: D
        type(VF),intent(in) :: Aun
        integer,intent(in) :: col,component
        ...
        if (row.eq.col) then; D%x%RF(t)%f(i,j,k) = Aun%x%RF(t)%f(i,j,k); found = .true.; exit; endif
        row = row + 1
        enddo; enddo; enddo; enddo
        !$OMP END PARALLEL DO
        case (2)
        ...
        if (row.eq.col) then; D%y%RF(t)%f(i,j,k) = Aun%y%RF(t)%f(i,j,k); found = .true.; exit; endif
        row = row + 1
        enddo; enddo; enddo; enddo
        !$OMP END PARALLEL DO
        case (3)
        ...
        if (row.eq.col) then; D%z%RF(t)%f(i,j,k) = Aun%z%RF(t)%f(i,j,k); found = .true.; exit; endif
        row = row + 1
        enddo; enddo; enddo; enddo
        !$OMP END PARALLEL DO
        case default; stop 'Error: dir must = 1,2,3 in define_ith_diag_VF in matrix.f90'
        end select
        if (.not.found) stop 'Error: diagonal element not found.'
      end subroutine
