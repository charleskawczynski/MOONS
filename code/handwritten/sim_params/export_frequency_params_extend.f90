       ...
       subroutine init_EFP(EFP,export_ever,export_first_step,frequency_coeff,&
         frequency_base,frequency_exp,t_window_start,t_window_stop)
         implicit none
         ...
         real(cp),intent(in) :: t_window_start,t_window_stop
       ...
       subroutine update_EFP(EFP,t,n_step,substep)
         implicit none
         type(export_frequency_params),intent(inout) :: EFP
         real(cp),intent(in) :: t
         ...
         if ((EFP%export_ever).and.((t.gt.EFP%t_window_start).and.(t.lt.EFP%t_window_stop))) then
