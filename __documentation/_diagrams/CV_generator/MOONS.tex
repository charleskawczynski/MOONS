...
\input{../includes.tex}
\input{../../includes/diagrams/coordinates.tex}
...
% Edge fields:
	...
	\draw[line width=\wArr,\arrR] ({\ZEDGEDX},{\ZEDGEDY}) -- ++ ({-\arrowL*\DIAGX},{-\arrowL*\DIAGY})  node[below=-.4,anchor=north west]  {\EQS $(\sigma,J_z)_{i+\frac{1}{2},j+\frac{1}{2},k}$};

% Coordinate system and cell center fields (coordinate system cell center)
	% \draw [fill] (\CCX,\CCY) circle [radius=0.02];
	% \draw (\CCX,\CCY) node[right=1.3,anchor=north]{\EQS$ (p,\cleanB)_{i,j,k}$};
...
% Coordinate system and cell center fields (coordinate system outside)
	\draw [fill] (\CCX,\CCY) circle [radius=0.02];
	% \draw (\CCX,\CCY) node[left=.5,anchor=east]{\EQS$ (p,\cleanB)_{i,j,k}$};
	\draw (\CCX,\CCY) node[left=.7,anchor=south]{\EQS$ (p,\cleanB)_{i,j,k}$};
